...
- Order-list model pinned to the staged erratum (`s3m-position-jump-pattern-break-and-adpcm.md` §Erratum, added for docs #262): `Bxx` indexes the **raw** on-disk order list (the number ST3's own order editor displays), in-band sentinels are resolved at fetch time — `254` ("++") skipped forward, `255` ("--") ends the tune per the ScreamTracker v3.20 format doc — and the list is never marker-compacted. This was already the implementation's behaviour; it is now locked by a regression battery transcribing the erratum's worked probe table (`B04`/`B05`/`B06` on `[000,001,002,002,254,254,003,004]` all land on pattern 003; `B07` keeps raw slot 7 → pattern 004 instead of the compacted model's wrap; a `Bxx` onto the end mark ends the tune; an out-of-range `Bxx` ends the decode — the finite-decode mapping of the live player's documented wrap-to-start). New `PlayerState::divergent_jump_count` diagnostic implements the erratum's recommendation to "surface any Bxx that lands on or after the first sentinel": each `Bxx` naming a slot at/after the first `254`/`255` (or past the end of the list) increments it, since that is exactly the set of modules on which the raw-list and compacted models pick different patterns. SBx loop-backs never trip it.
...
- effect memory no longer applies to Axx / Bxx / Cxy / Txx / Vxx — they are unmarked (no `%`/`*`) in the multimedia.cx effect list and take their infobyte literally: A00/T00 are ignored per their own rules, B00 jumps to order 0, C00 breaks to row 0, and V00 sets global volume to 0 instead of recalling a stale parameter
- vibrato/tremolo waveform 3 ("random"): now reuses the sine table instead of generating noise, matching real ST3 — FireLight §6.8 / §6.15 `case 3: delta = sintab[temp]; // random - just use sine.` ST3 never shipped an actual noise LFO, so an `S33` / `S43` waveform-select modulates pitch/volume identically to the default sine rather than producing audible noise. The unused per-channel LCG state was removed
- Vxx (global volume): per-voice latched model — a Vxx change no longer retroactively rescales already-playing voices. Each voice latches the global volume live at its last note-volume write (trigger, volume column, instrument reload incl. SDx-deferred, or any active-volume effect: Dxy/Kxy/Lxy slide, Qxy modifier, Rxy tremolo, Ixy tremor); the mixer scales each voice by its own latched value rather than a single mix-down scalar. Same-row triggers keep the pre-Vxx value (Vxx drains on tick 1); later-tick volume changes pick up the post-Vxx value (multimedia.cx §Vxx: "does not affect past notes ... unless their volume is changed")
- vibrato (Hxy/Uxy/Kxy): phase now follows the documented 256-cycle convention — stepped by `speed * 4`, sampled at `phase / 4` — mirroring the (already-correct) Rxy tremolo path. The prior `(pos + speed*4) & 0x3F` form masked to 6 bits while sampling the table directly, advancing the pointer 4× too fast and quartering the vibrato period (multimedia.cx §Playback Notes; FireLight §6.8 signed `-32..+31` pointer incremented by `speed`)
